...
sub exec_item {
	...
	$self->schema->txn_do(
		sub {
			...
			$self->logger->debug(sprintf("[donation_id=%s] [decred_data_digest=%s] Timestamping.",$donation->id,$decred_data_digest)) if $self->has_log;
			...
			if (%update_data) {
				...
				$self->logger->debug(sprintf("[donation_id=%s] [decred_merkle_root=%s] [decred_capture_txid=%s]",$update_data{decred_merkle_root}  || 'undef',$update_data{decred_capture_txid} || 'undef',)) if $self->has_log;
