package VotoLegal::Controller::PublicAPI::Donation::MerkleRoot;
use common::sense;
use Moose;
...
sub root : Chained('/publicapi/donation/base') : PathPart('') : CaptureArgs(0) {
    my ( $self, $c ) = @_;

    $c->stash->{collection} = $c->model('DB::VotolegalDonation')->search(
        {
            'me.refunded_at'         => \'IS NULL',
            'me.decred_capture_txid' => \'IS NOT NULL',
...
sub base : Chained('root') : PathPart('merkle_root') : CaptureArgs(0) { }
    ...
    return $self->status_ok(
        ...
        entity => [
            map {
                ...
                # TODO Verify and got timestamp.
                my $donations = $donations->{$decred_merkle_root}{donations};
