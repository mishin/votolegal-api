...
db_transaction {
    ...
    is( $schema->resultset('Project')->search( { candidate_id => $id_candidate } )->count, 1, 'project added' );

    # Editando o projeto.
    rest_put stash 'project.url',
      name   => "editing project",
      stash  => "e1",
      params => { title => "Project 01", },
      ;

    is( $schema->resultset('Project')->find( stash 'project.id' )->title, 'Project 01', 'project edited', );
    ...
    is( $schema->resultset('Project')->search( { candidate_id => $id_candidate } )->count, 0, 'project deleted' );
