...
db_transaction {
    ...
    for my $created_at (qw(2016-05-01 2016-09-01 2015-02-01)) {
        ok(
            $candidate->donations->create(
                {
                    %{ $fakeDonation->() }, created_at => $created_at,
                }
            ),
            'add donation',
        );
    ...
    stash_test "s1" => sub {
        ...
        is( $res->{donations_between_hundred_and_fourhundred_through_votolegal},
            1, 'donations between hundred and fourhundred' );
        is( $res->{donations_between_fourhundred_and_thousand_through_votolegal},
            1, 'donations between four hundred and thousand' );
    };
};
